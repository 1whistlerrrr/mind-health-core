...
# 🚀 用公开中文微调数据集训练！
# 支持：alpaca-zh、BELLE、firefly 等中文数据集
import torch
import os
...
OUTPUT_DIR = "./qwen-0.5b-lora-public-data"
MAX_SEQ_LEN = 512
TRAIN_BATCH_SIZE = 4
GRADIENT_ACCUMULATION_STEPS = 2
LEARNING_RATE = 1e-4
MAX_STEPS = 500  # 比之前多一点

# 选择要使用的数据集：'alpaca-zh' / 'belle' / 'demo'
DATASET_CHOICE = 'alpaca-zh'
...
# ============ 第 3 步：加载公开数据集！ ============
print(f"\n{'='*60}")
print("📖 【第 3 阶段】加载公开数据集...")
print(f"{'='*60}")

def format_example_alpaca(example):
...
if DATASET_CHOICE == 'alpaca-zh':
    print(f"\n🎯 选择使用: Alpaca-Chinese (5万条中文指令数据)")
    dataset_name = "shibing624/alpaca-zh"
    dataset = load_dataset(dataset_name, cache_dir="./hf_cache")
    print(f"数据集加载成功！共 {len(dataset['train'])} 条数据！")
    
    # 只用前 5000 条快速演示效果，避免训练太久
    dataset = dataset["train"].select(range(min(5000, len(dataset["train"]))))
    dataset = dataset.map(format_example_alpaca, remove_columns=dataset.column_names)
